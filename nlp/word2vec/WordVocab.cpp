...
void VocabWords::SaveVocab(const char* vocabFile,const vocab_word *vocabs, long long vocabSize,const char* trainFile, long fileSize) {
  FILE *fo = fopen(vocabFile, "wb");

  fprintf(fo,"%s% ld\n",trainFile,fileSize);

  for (long long i = 0; i < vocabSize; i++) fprintf(fo, "%s %lld\n", vocabs[i].word, vocabs[i].cn);

  fclose(fo);
}
bool VocabWords::ReadVocab(const char* fileName) {
  FILE *fin = fopen(fileName,"rb");
  if (fin == NULL) return false;
  char word[Params::MAX_WORD_LENGTH];
  TrainFileReader::ReadWord(word,fin);
  char c;
  fscanf(fin,"%ld%c",&mTrainFileSize,&c);
...
void VocabWords::Build() {
  char wordBuffer[Params::MAX_WORD_LENGTH];
  const char* readVocabFile = mParams->getReadVocabFile();
  if (readVocabFile[0] != 0) {
    ReadVocab(readVocabFile);
  }
  ...
  AddWord((char *) "</s>");
  TrainFileReader *fileReader = new TrainFileReader(mParams->getTrainFile());
  mTrainFileSize = fileReader->getFileSize();
  while (fileReader->ReadWord(wordBuffer)) {
    ...
    if (vocab_size > vocab_hash_size * 0.7) {
      ReduceVocab(mParams->getMinCount());
    }
  ...
  if (Params::getInstance()->getDebugMode() > 0) {
    printf("Vocab size: %lld\n", vocab_size);
    printf("Words in train file: %lld\n", train_words);
  }
  const char* saveVocabFile = mParams->getSaveVocabFile();
  //Save the vocab file.
  if (saveVocabFile[0] != 0){
    SaveVocab(saveVocabFile,mVocabs,vocab_size,mParams->getTrainFile(),mTrainFileSize);
  }
 ...
 * 'vocab' table.
 * if read the end of the file , return false;
 * else return true.
 */
/*
bool VocabWords::ReadWordIndex(TrainFileReader* fileReader, int& index) {
  if (fileReader == 0) return false;

  char word[Params::MAX_WORD_LENGTH];
  if (fileReader->ReadWord(word,fp)) {
